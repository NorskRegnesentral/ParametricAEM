...
file_path = system.file("extdata",
                        "Mackerel_otoliths.csv",
                        package = "ParametricAEM")
data_mackerel_otoliths = read.csv(file_path, sep = ",")
data_exp = data_mackerel_otoliths[data_mackerel_otoliths$expertise == 1, ] # Extract experts
data_exp = data_exp[data_exp$age > 0, ]
data_analysis = Compute_modes(data_exp)
data_analysis_capped = Cap_ages(data_analysis, min_age = 1, max_age = 12)
...
best_matrix = Calculate_matrix(all_pars, min_age = 1, max_age = 12)
