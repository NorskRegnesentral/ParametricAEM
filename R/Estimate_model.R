...
Estimate_model = function(data, min_age, max_age, start = c(2, 0, 0.5, 0, 0.2), estimate = rep(1, 5)){
    ...
    par = c(par.est, par.fixed)
    names(par) = parnames
    fit = stats::nlminb(par.est, negloglik, data = data, par.fixed = par.fixed, min_age = min_age, max_age = max_age, parnames = parnames)
    results = list()
    Sigma = solve(numDeriv::hessian(negloglik, x = fit$par, data = data, par.fixed = par.fixed, min_age = min_age, max_age = max_age, parnames = parnames))
...
negloglik = function(param, data, par.fixed, min_age, max_age, parnames){
    par = c(param, par.fixed)
    return(-loglik(par, min_age = min_age, max_age = max_age, data = data))
}
...
#'
loglik = function(theta, min_age, max_age, data){
    beta0 = theta["beta0"]
    beta1 = theta["beta1"]
    alpha0 = theta["alpha0"]
    alpha1 = theta["alpha1"]
    phi = theta["phi"]
    true_age = data$modal_age_closest # data$modal_age_low ## if using the lowest mode as true age
    age = data$age
    weight = data$weight
    totLik = 0
